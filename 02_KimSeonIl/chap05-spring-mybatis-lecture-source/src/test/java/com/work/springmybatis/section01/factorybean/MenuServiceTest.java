...
/* 단위 테스트
* - Mokito를  활용하여 의존성을 모킹(Mocking) 후 MenuService의 순수 비즈니스 로직을 검증
*
* - Mokito : 가짜 객체(Mock)를 만들어서 해당 객체가 어떤 상황에서 어떤 값을 반환할지 정의할 수 있게 하는 테스트 프레임워크*/
...
class MenuServiceTest {
  ...
  @InjectMocks  // 모의객체를 MenuService에 주입 (DI 대체)
  private MenuService menuService;  //

  @BeforeEach //테스트 메서드가 매번 수행되기 전에 실행
  public void setUp(){
    Mockito.when(sqlSession.getMapper(MenuMapper.class)).thenReturn(menuMapper);
  }
  ...
  void findAllMenuByOrderableStatus_Y() {
    ...
    MenuDTO menu1 = new MenuDTO(1, "김치찌개", 9000, 1, orderableStatus);
    MenuDTO menu2 = new MenuDTO(1, "된장찌개", 8000, 1, orderableStatus);

    List<MenuDTO> originalList = Arrays.asList(menu1, menu2);
    /*모의 객체 menuMapper의 메서드 수행시 originalList가 반환하도록 설정
    * (orginalList가 DB조회 결과)*/
